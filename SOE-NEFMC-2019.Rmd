...
```{r management-objectives, results='asis'}
readd(man_obj)
```
...
gom_landings <- landings %>% filter(EPU == "GOM") %>% 
  ...
  #Facet 
  facet_wrap(feeding.guild~.,scales = "free_y", labeller = label_func, ncol = 1) +
...
rec_div <- recdat %>% 
 ...
 #Highlight last ten years
  ...
  scale_color_manual(values = "black", aesthetics = "color")+
  guides(color = FALSE) +
...
ggplot(aes(x = Time, y = Value, color = Var)) +
  ...
  scale_color_manual(values = rep("black",length(unique(total_surv$Var))),
                     aesthetics = "color")+
  guides(color = FALSE) +
  geom_hline(aes(yintercept = hline,
                 group = Var),
...
ggplot(aes(x = Time, y = Value, color = Var)) +
  ...
  geom_line(size = lwd-0.5) +
  geom_point(size = pcex-0.5) +
  scale_color_manual(values = rep("black",length(unique(total_surv$Var))), aesthetics = "color")+
  guides(color = FALSE) +
  geom_hline(aes(yintercept = hline,
                 group = Var),
             size = hline.size,
